...
Steps for cloning, creating a branch, and pushing on the server (<server>).
...
```
[Your home directory]               [Shared area on the server]
Local repository  <-- clone/pull -- Bare repository (/path/to/repository/)
                  ---- push ------>
```

**Characteristics of a bare repository:**
- Contains only the contents of `.git/` — files cannot be edited directly
- By convention, the folder name ends with `.git` (e.g., `example.git`)
...
Your prompt should read `<username>@<server>:~/example$`.

### 1-2. Locate the bare repository

On the server, bare repositories are stored under `/path/to/repository/`.
...
```bash
git clone /path/to/repository/<username>/example.git
```
...
```bash
ls
# example should no longer appear
```
...
|-----------------|-------------------------------|
| Local clone (`~/example/example`) | **No impact.** The bare repository on the server remains intact |
| The bare repository itself (`/path/to/repository/...`) | Everyone is affected. **Do not touch it** |
