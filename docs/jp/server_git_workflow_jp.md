...
サーバー（<server>）でのクローンからブランチ作成・プッシュまでの手順。
...
```
[自分のホームディレクトリ]              [サーバー上の共有領域]
ローカルリポジトリ  <-- clone/pull --  ベアリポジトリ (/path/to/repository/)
                   ---- push ------>
```

**ベアリポジトリの特徴：**
- `.git/` の中身だけが存在し、ファイルを直接編集できない
- 慣例でフォルダ名の末尾に `.git` をつける（例：`example.git`）
...
サーバーでは `/path/to/repository/` 以下にベアリポジトリが置いてある。
...
| 削除した対象 | ベアリポジトリへの影響 |
|------------|----------------------|
| ローカルのクローン（`~/example/example`） | **影響なし**。ベアリポジトリはサーバー上にそのまま残る |
| ベアリポジトリ自体（`/path/to/repository/...`） | 全員が影響を受ける。**触らない** |
